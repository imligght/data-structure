...
structures::LinkedList< structures::LinkedList<T> * >
    ...
    LinkedList<T>* l_evens = new LinkedList<T>();
    LinkedList<T>* l_odds = new LinkedList<T>();

    for (std::size_t i = 0; i < size(); i++) {
        if (i % 2 == 0) {
            l_evens->push_back(at(i));
        } else {
            l_odds->push_back(at(i));
        }
    }

    list_halve.push_back(l_evens);
    list_halve.push_back(l_odds);
